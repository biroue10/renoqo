...
export function trackQuote(event:QuoteEvent,properties?:SafeProperties){if(typeof window==="undefined")return;try{window.renoqoQuoteAnalytics?.(event,properties);if(event===QUOTE_EVENTS.started)trackEvent("quote_form_started");if(event===QUOTE_EVENTS.submitted)trackEvent("quote_submitted",{status:typeof properties?.status==="string"?properties.status:undefined});}catch{}}
